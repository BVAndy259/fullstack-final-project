...
const Inventory = () => {
  ...
  const [tab, setTab] = useState('movimientos'); // 'movimientos' | 'entrada' | 'ajuste' | 'alertas'
  const [alerts, setAlerts] = useState([]);
  const [form, setForm] = useState({ product_id: '', quantity: '', new_stock: '', reason: '' });
  ...
  const fetchAll = async () => {
    const [movRes, prodRes, alertRes] = await Promise.all([
  ...
  const handleStockEntry = async (e) => {
    ...
    try {
      ...
      setForm({ product_id: '', quantity: '', new_stock: '', reason: '' });
      fetchAll();
      setTab('movimientos');
    } catch (err) {
